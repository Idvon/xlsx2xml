...
def generate_xml_from_data(header: dict, post_header: dict, items: list, sum_v: dict, output_path: Path) -> None:
    ...
    doc.set('ПоФактХЖ', 'Документ об отгрузке товаров (выполнении работ),'
    ...
    for item in items:
        ...
        _make_sub_element(dop_sved, 'КрНаимСтрПр', text=str(item.get('КрНаимСтрПр', '')))
        ...
        _make_sub_element(sved_pros, 'НомТовПрослеж', text=str(item.get('НомТовПрослеж', '')))
        _make_sub_element(sved_pros, 'ЕдИзмПрослеж', text=str(item.get('ЕдИзмПрослеж', '')))
        _make_sub_element(sved_pros, 'НаимЕдИзмПрослеж', text=str(item.get('НаимЕдИзмПрослеж', '')))
        _make_sub_element(sved_pros, 'КолВЕдПрослеж', text=_fmt(item.get('КолВЕдПрослеж', ''), 3))
        _make_sub_element(sved_pros, 'СтТовБезНДСПрослеж', text=_fmt(item.get('СтТовБезНДСПрослеж', ''), 2))
    ...
    sv_per.set('СодОпер', 'Товары переданы, работы выполнены, услуги оказаны в полном объеме')
    ...
    fio_podp.set('Отчество', header.get('ОтчествоПодписант', header.get('Отчество', ' ')))
